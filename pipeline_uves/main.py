...
for zidx in range(opt.zbinlen):
    ...
    for i in range(nqso):
        ...
        #print(name, np.sum(qso_arr[i].flux[mask])/np.sum(mask), np.sum(mask))
...
print("mean fluxes = ", mf_output_df.mf_a, mf_output_df.mf_tot)
#exit()
...
for zidx in range(opt.zbinlen): #aug20
    ...
    for qidx in range(nqso): #aug20
        ...
        kmax = [100,100,100] # [qso_arr[qidx].kmax[Lya_str], qso_arr[qidx].kmax[Lyb_str], 2**.5*(1./qso_arr[qidx].kmax[Lya_str]**2 +1./qso_arr[qidx].kmax[Lyb_str]**2)**-0.5] #temporary
