...
class QuasarSpectrum(object):
    ...
    def load_cat(cls,tag):
        ...
        if tag.startswith('obs'): #What matters is what the tag starts/ends with. This is loading a catalog for OBS
            path_to_cat = "../data/obs/UVES_DR1_QSO_catalogue.txt"
            catalog_table = pd.read_csv(path_to_cat,delim_whitespace=True,usecols=(3,6),
                                names = ("qso_name","redshifts"), comment='#', skiprows = 355) #only z>3 quasars

            #print(catalog_table['qso_name'], catalog_table['redshifts'])
            #exit()
            path_to_dlas = "../data/obs/UVES_DR1_DLA_catalogue.txt"
            dla_colnames = ['idx','name','z','NHI','err_NHI']
            ####
            dla_table = pd.read_csv(path_to_dlas,delim_whitespace=True,names=dla_colnames)

            path_to_D1 = "../data/obs/DR1_quasars_master.csv"
            D1_table  = pd.read_csv(path_to_D1) #,names=['qsonum', 'catelog', 'longname', 'name', 'RA', 'DEC', 'z_qso', 'aperture', 'seeing_min', 'seeing_max']
     
            
            qname_array,z_array = catalog_table['qso_name'].values,catalog_table['redshifts'].values
            nqso = 100
            cls.dla_table = dla_table
            cls.qso_table = D1_table
            #print(int(str(cls.qso_table['CNR'][cls.qso_table.Name_Adopt == 'J000149-015939']).split(',')[2]))
            #exit()
    ...
    def load_qso_data(cls,cat_index, tag, rescale_flux):
        ...
        if name.startswith("mock"):
            ...
            released_path = "../data/obs/uves/spectra/{0}_uves.txt".format(name)
            ...
            #cont_table = pd.read_csv(continuum_path, delim_whitespace=True, skiprows = 1,usecols=(0,1),names = ("wav",'flx'))
            mask_wave = (qso_table.wav.values>opt.lyb_min*(1+redshift))&(
                ...
                (qso_table.ferr.values)>opt.min_trans)  #Matt M: Second criteria is because opt.lyb_max is the highest redshift pixel that is used (and I suspect we keep Lyman-alpha for it in case we want to regenerate forest)
            ...
            flux =  qso_table.flx.values #unnormalized_flux/cont_table.flx.values
            ...
            unnormalized_flux=np.ones_like(qso_table.flx.values)*np.nan
            #print(name,redshift, len(flux))
            #exit()

            CNR = int(str(cls.qso_table['CNR'][cls.qso_table.Name_Adopt == name]).split(',')[2])
            print("name", name, "CNR(l=5500A)=", CNR)
            if CNR < inis.CNR_threshold:
                mask_dla *= False 
            ...
            if inis.uves_resolution: #feb27
                resolution =  opt.R_UVES
       #     else:
        #        m1 = wavelength>= opts.overlap_maxwav. #Carswell+18 #this is exactly where the overlap stops and it becomes the VIS arm
        ...
        dloglambda = qso_table.dloglam.values/np.log(10)
