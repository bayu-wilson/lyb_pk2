...
labels = [r"$\widehat{P}_{\alpha \alpha}$",r"$\widehat{P}_{TT}$",
        r"$\widehat{P}_{\alpha \beta}$",r"$\widehat{\cal{P}}_{\beta \beta}$",
        r"$\left|\widehat{Q}_{\alpha \beta}\right|$"]
...
plt.tight_layout()
if inis.save_quick_plot_pk:
    plt.savefig(inis.save_quick_plot_pk_path)
if show_plot:
    plt.show()
else:
    plt.clf()
