    ...
    for j in range(3):
        if zidx<7:
            ...
            if (i == 0)&(j==2):
                ...
                # print(np.log10(k*paa/np.pi),np.log10(err_paa*k/np.pi))
                # print((k*paa/np.pi),(err_paa*k/np.pi))
                # print('\n\n\n')
                ...
                ax2 = ax[i,j].twinx()
                #ax2.set_yscale('log')
                ax2.plot(k_x,N_aa,color=colors[0])
                ax2.plot(k_x,N_tt,color=colors[1])
                ax2.plot(k_x,N_ab,color=colors[2])
...
ax[0,1].text(1.2, 0.5,"Number of Pixels", ha='center', va='center', transform=ax[0,1].transAxes,fontsize=20,rotation=-90)
