...
pk_path_list =["../output/pk_errboot_obs_lyb_nocorr_DLATrue_metalFalse_boot0.csv",
                "../output/pk_errboot_obs_lyb_nocorr_DLAFalse_metalFalse_boot0.csv"]
pk_path_list =  ["../output/pk_obs_lyb_nocorr_DLATrue_metalFalse.csv",
                    "../output/pk_obs_lyb_nocorr_DLAFalse_metalFalse.csv"]



obs_names = ["k","z","Paa","Ptt", "Pab", "errPaa", "errPtt", "errPab"]
boot_pk_err = pd.read_csv("../output/pk_errboot_obs_lyb_nocorr_DLAFalse_metalFalse_boot0.csv",delim_whitespace=True, names = obs_names)
...
for i,zidx in enumerate(range(4,7),start=0):
    ...
    if plot_obs:
        ...
        #print("zmask = ",  zidx, opt.zbin_centers,d_arr[0].z[1])
        ...
        ax[m].plot(k_x,t_removing_dla.ptt/t_remaining_dla.ptt,color=colors[0])
        ax[m].plot(k_x,t_removing_dla.npix_tt/t_remaining_dla.npix_tt,color=colors[0],ls='--')

        #if opt.zbin_centers[zidx] == 4.2:
        #    print("z=4.2:  ", k_x, t_removing_dla.npix_aa -t_remaining_dla.npix_aa)
...
if inis.save_ratio_dla_fig:
    print("printing ", inis.save_ratio_dla_fig_path)
