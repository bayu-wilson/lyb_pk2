...
####################################################################################################
labels = [r"$\left| \widehat{Q}_{\alpha \beta} / \widehat{P}_{\alpha \beta} \right|$"]
...
pkdata = pd.read_csv(inis.save_pk_path)#pd.read_csv("../final_results/pk_errboot_obs_corrNR.txt")
...
fig.text(0.02, 0.5, r"$\left|P_{xx}/P_{yy}\right|$", ha='center', va='center', rotation='vertical',fontsize=25)
plt.grid(False)
...
for i in range(2):
    for j in range(3):
        if zidx<7:
            ...
            t = pkdata[pkmask]
            k,paa,ptt,pab,qab,pbb = t.k.values,t.paa.values,t.ptt.values,t.pab.values,t.qab.values,t.pbb.values
            #err_paa,err_ptt,err_pab,err_qab,err_pbb= (t.err_paa.values, t.err_ptt.values,
            #                                          t.err_pab.values,t.err_qab.valuest.err_pbb.values)
            ...
            if (i == 0)&(j==2):
                ...
                ax[i,j].errorbar(k_x,np.abs(qab/pab),color=colors[0],marker='.')
                #print(qab/pab)
                # ax[i,j].errorbar(k_x,np.log10(k*paa/np.pi),color=colors[0],marker='.')
                # ax[i,j].errorbar(k_x*0.99,np.log10(k*ptt/np.pi),color=colors[1],marker='.')
                # ax[i,j].errorbar(k_x,np.log10(k*pab/np.pi),color=colors[2],marker='.')
                # ax[i,j].errorbar(k_x,np.log10(k*pbb/np.pi),color=colors[3],marker='.')
                # ax[i,j].errorbar(k_x,np.log10(k*np.abs(qab)/np.pi),color=colors[4],marker='.')
...
ax[1,0].legend(custom_lines[:2],labels[:2],loc='upper right',ncol=1,frameon=False)
# ax[1,1].legend(custom_lines[2:4],labels[2:4],loc='lower left',ncol=1,frameon=False)
# ax[1,2].legend(custom_lines[4:],labels[4:],loc='lower left',ncol=1,frameon=False)
...
plt.tight_layout()
if inis.save_quick_imaginary_ratio:
     plt.savefig(inis.save_quick_imaginary_ratio_path)
if show_plot:
    plt.show()
else:
    plt.clf()
