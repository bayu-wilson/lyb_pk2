...
####################################################################################################
labels = [r"$\widehat{P}_{\alpha \alpha}/ \widehat{P}_{\alpha \alpha,metals \, subtracted}$",
            r"$\widehat{P}_{TT}/\widehat{P}_{TT,metals \, subtracted}$",
            r"$\widehat{P}_{\alpha \beta}/\widehat{P}_{\alpha \beta, metals \, subtracted}$",
            r"$\widehat{\cal{P}}_{\beta \beta} /\widehat{\cal{P}}_{\beta \beta, metals \, subtracted}$"]
...
######### METAL POWER HERE #########
metal_power = np.concatenate([np.loadtxt('../data/obs/pk_xs_avg.txt')]) #subtracting metals again
...
for j in range(3):
    if zidx<7:
        ...
        #print(len(metal_power),len(paa))
        paa_metals = paa + metal_power
        ptt_metals = ptt + metal_power
        pab_metals = pab + 0.
        pbb_metals = pbb + 0.
...
plt.tight_layout()
if inis.save_paper_pk_v4_metal_power_ratio:
    plt.savefig(inis.save_paper_pk_v4_metal_power_ratio_path)
if show_plot:
    plt.show()
else:
    plt.clf()
