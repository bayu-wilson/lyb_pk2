...
# PLOTTING VID'S AND MY POWER SPECTRUM MEASUREMENTS. (X,Y) AXES: (P_[BAYU,VID],K)
# OUTPUT: "figures/paper_compare_pk_bv.pdf"
    ...
    for j in range(3):
        if zidx<7:
            ...
            if (i == 0)&(j==2):
                ...
                k_x = np.log10(k)#*1.01
                ax[i,j].plot(k_x,k*paa/np.pi,color=colors[0],alpha=0.2)
                zidx += 1
                #print(k)
...
plt.tight_layout()
if inis.save_paper_compare_pk_bv:
    plt.savefig(inis.save_paper_compare_pk_bv_path)
if show_plot:
    plt.show()
else:
    plt.clf()
